    ...
    public static void main(String[] args) {
        ...
        do {                                                    //do-while loop so the program keeps repeating
            ...
            try {                                               //try catch block for numerator in case bad data is entered
                ...
                if (quitChecking.charAt(0) == 'q' || quitChecking.charAt(0) == 'Q') {       //if the bad data starts with "q"/"Q" then terminate from the loop and therefore program
                    break;
                }

                System.out.println("You entered bad data.");    //informative message telling user they entered bad data and restarting the loop
            ...
            try {                                               //try catch block for divisor in case bad data is entered 

                System.out.println("Enter the divisor:");       //asks for the divisor
                divisor = input.nextDouble();

            } catch (InputMismatchException e) {                        //catches bad data
                System.out.println("You entered bad data.");            //informative message telling them they entered bad data and restart the loop
                System.out.println("Please try again.");
                input.nextLine();
                continue;
            }

            if (divisor == 0) {                                                 //if the user entered zero for the divisor, tell them they can't do that and restart the loop
