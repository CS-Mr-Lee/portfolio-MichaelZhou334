...
public class Zhou_Michael_ChampionFilter {
    ...
    public static void main(String[] args) {
        ...
        try {               //try-catch block in case file doesn't exist or cannot read/write the file data
            ...
            while (lineIn != null) {
                lineIn = lineIn.replaceAll("\\s", "");      //finds all the empty space and replaces it with nothing, so the String.split can work properly
        ...
        } catch (FileNotFoundException e) {           //catch if the files is not found
            e.printStackTrace();
        } catch (IOException e) {                     //catch if there is bad data in the files
            e.printStackTrace();
